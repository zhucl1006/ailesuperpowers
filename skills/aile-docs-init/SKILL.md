---
name: aile-docs-init
description: 项目文档结构创建助手。作为资深产品经理，通过多轮对话深入了解项目需求，分析合理性并提供最佳实践建议，最终生成完整的技术文档体系。
---
...
模式 B 不应该只产出“项目简介 + 模块概述”，而应默认生成可追溯、可维护的工程档案。

**默认要求：**

1. 默认使用**详细版**文档，除非用户明确要求精简版
2. 每份文档都必须区分：
   - `代码提取`：可直接从代码、配置、测试、迁移中得到
   - `用户补充`：需要用户提供的业务背景或规划
   - `合理推断`：无法直接证明，但可基于代码结构推断，必须说明依据
3. 每个核心模块文档都必须包含**代码详单**，至少覆盖：
   - 文件/目录清单
   - 对外导出接口
   - 关键调用链或时序
   - 依赖与被依赖关系
   ...
   - 测试现状与缺口
4. 模式 B 除 `PRD.md`、`SAD.md` 外，强烈建议额外生成
   - `docs/specs/CODEBASE-ANALYSIS.md`
     用于承接代码仓地图、运行时入口、接口盘点、数据模型盘点、配置矩阵、技术债清单
5. 禁止使用“若干”“一些”“负责某类逻辑”之类的空泛描述，必须尽量写出路径、接口名、数据结构、调用关系和实现状态
6. 对于无法确认的内容，必须集中记录在“待确认事项”中，不能把猜测写成事实
...
- 找出入口文件和核心模块
- 理解项目的整体结构
- 输出目录树摘要（不要只是列目录名，要标注职责）
- 标记运行时入口、构建入口、测试入口、脚本入口
```
...
模式 B 必须先沉淀一份“代码证据画像”，后续文档都基于这份画像展开。
...
**默认规则**：模式 B 如果用户没有明确选择，默认按“详细版”执行。
...
将要生成的文档:
  ...
  ✓ docs/specs/CODEBASE-ANALYSIS.md - 代码详单（基于代码证据画像）
...
- 模块列表（从代码分析）
- 文档索引
- 若生成 `CODEBASE-ANALYSIS.md`，在 README 中标记为“代码回补必读”
...
- 核心功能描述（从代码功能逆向生成）
- 功能列表（基于代码实现的功能）
- 功能实现矩阵（功能 / 状态 / 关键入口 / 证据路径 / 待确认点）
- 用户旅程 / 业务流程（从代码行为和页面/接口流转推断）
- 技术实现说明（标注已实现）
- 已知问题和改进点（从代码分析得出）
- 待确认事项（缺少业务信息、无法从代码得出的背景）
...
```markdown
- 来源：代码提取
- 证据：src/auth/service.ts, src/auth/routes.ts
- 可信度：高
```
...
### 4.4.1 生成 docs/specs/CODEBASE-ANALYSIS.md（模式 B 强烈建议）

使用模板 `./docs-templates/CODEBASE-ANALYSIS-template.md`，填充：

**模式 B（从代码回补）：**
- 仓库结构总览（目录树 + 职责）
- 运行时入口 / 构建入口 / 测试入口
- 核心模块映射（模块 -> 文件/目录 -> 职责）
- 外部契约盘点（HTTP/API/CLI/Job/Webhook）
- 数据资产盘点（实体、schema、migration、缓存）
- 配置矩阵（环境变量、配置文件、默认值来源）
- 质量信号（测试、lint、类型检查、脚本）
- 技术债与高风险区域
- 待确认事项与建议补访谈问题
...
docs/modules/{module-name}.md
  - 模块概述（从代码分析）
  - 职责范围（从代码职责分析）
  - 文件/目录详单（关键文件职责、入口、导出）
  - 接口定义（从代码导出分析）
  - 依赖关系（从 import 分析）
  - 关键调用链 / 时序（请求如何进入并流经模块）
  - 数据结构 / 状态流转
  - 配置项 / 环境变量 / Feature Flag
  - 实现说明（关键逻辑说明）
  - 已有测试（从测试代码分析）
...
docs/api/api-spec.md（从代码生成）
  ...
  - 接口列表（从路由文件提取）
    ...
    - 响应格式（从代码分析）
    - 错误码（从错误处理代码提取）
    - 实现入口（controller/service/handler）
  - 通用错误处理
  - API 版本管理（如有）
```
...
3. **识别差异**：指出代码与最佳实践的差异
4. **记录债务**：记录技术债务和改进建议
5. **证据可追溯**：关键结论尽量绑定到具体文件/目录/接口
6. **详单优先于概括**：模块文档必须包含文件级详单，而不是只有职责描述
7. **状态显式化**：标记“已实现 / 部分实现 / 待确认 / 推断”
