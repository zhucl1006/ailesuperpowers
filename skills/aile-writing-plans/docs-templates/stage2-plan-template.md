...
- 若 `analysis.md` 已提出「檔案系統回補建議」，本模板必須將其轉成顯式執行任務。
...
| 文件 ID | 文件路徑 | 回補內容 | 來源條目 | 驗收方式 | 負責人 | 狀態 |
|--------|----------|----------|---------------|--------|------|
| D1 | `docs/modules/...` | [示例：補充設定說明] | analysis: 檔案系統回補建議 1 | 人工檢查 + PR Review | {Owner} | 待開始 |
| D2 | `docs/specs/...` | [示例：同步業務規則變更] | analysis: 檔案系統回補建議 2 | 人工檢查 + PR Review | {Owner} | 待開始 |

### 檔案系統回補完成標準

- [ ] 對外行為變更已寫入使用文件
- [ ] 設定或介面變更已同步到模組文件
- [ ] `analysis.md` 中建議回補的檔案已全部覆蓋，或明確標記不處理原因
- [ ] 驗收/測試結論已同步到計劃或 Jira
